...
export default function EditBooking(){
    ...
    const fetchData = async() => {
        const url = baseURL + '/Booking/GetBookingById/'+id;
        const result = await httpGet(url);
        if(result.isSuccess){
            var data = result.data;
            setInmateId(data.inmateId);
            setBookingLocation(data.bookingLocation);
            setFacilityName(data.facilityName);
            setCharges(data.charges);
            setReleasedInformation(data.releasedInformation);
            setBookedDate(data.bookedDate.split('T')[0]);
            setReleasedDate(data.releasedDate?.split('T')[0]);
        }
    }
    useEffect(()=>{
        fetchData();
    ...
    async function handleSubmit(){
        ...
        const url = baseURL + "/Booking/UpdateBooking/"+id;
        ...
        <>
            <MainCard title="Edit Booking">
                ...
                <div style={{marginTop:"0.9rem"}}>
                    ...
                    <TextField id="outlined-basic" type="date" value={bookedDate} variant="outlined" autoComplete='off' fullWidth onChange={(e)=> setBookedDate(e?.target?.value)}/>
                </div>

                <div style={{marginTop:"0.9rem"}}>
                    <span>Released Date</span>
                    <TextField id="outlined-basic" type="date" value={releasedDate} variant="outlined" autoComplete='off' fullWidth onChange={(e)=> setReleasedDate(e?.target?.value)}/>
                </div>
                <div style={{marginTop:"0.9rem"}}>
